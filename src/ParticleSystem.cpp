...
void ThrustForce::set(const ofVec3f &dir) { 
	direction = dir; 
}
...
//Impulse Force
ImpulseForce::ImpulseForce(const ofVec3f f) {
	applyOnce = true;
	applied = true;
	force = f;
}
void ImpulseForce::set(const ofVec3f &dir) {
	force = dir;
}
void ImpulseForce::apply(const ofVec3f f) {
	applied = false;
	force = f;
}
void ImpulseForce::updateForce(Particle *particle) {
	particle->forces += ofGetFrameRate() * force;
}